...
export class FilterManager extends Cache {
    ...
    get (id: number): FilterType | undefined {
        const filter = this.filtersSet.findEntry(f => f.get(id) !== undefined)?.[0].get(id);
        return filter;
    }

    uninstall (id: number): boolean {
        const filter = this.filtersSet.findEntry(f => f.get(id) !== undefined)?.[0];
        const lastpoll = this.lastPollsSet.findEntry(f => f.get(id) !== undefined)?.[0];
